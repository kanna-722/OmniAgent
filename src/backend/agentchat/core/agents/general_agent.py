        ...
        while self._buffer:
            if self._inside_dsml:
                ...
                if end_match is None:
                    # Keep the incomplete protocol block private until its end arrives.
            ...
            # This '<' starts ordinary content rather than a DSML marker.
...
def _parse_dsml_parameter(value: str, attrs: str) -> Any:
    """Convert a DSML parameter to the closest JSON-compatible value."""
...
def recover_dsml_tool_calls(content: Any) -> tuple[str, List[Dict[str, Any]]]:
    """Recover tool calls emitted as Qwen/DashScope DSML text.

    A compatible OpenAI endpoint should return tool calls as structured metadata.
    Some model/provider combinations instead leak the internal DSML representation
    into ``content``.  LangGraph then treats it as a final answer and never reaches
    the tool node.  This parser is deliberately a narrow compatibility fallback;
    native structured tool calls remain the preferred path.
    """
    ...
    async def aafter_model(
        ...
        if not last_message.tool_calls:
            ...
            if recovered_tool_calls:
                logger.warning(
                    "Recovered {} DSML tool call(s) from model content: {}",
        ...
        try:
            async for token, metadata in self.react_agent.astream(
                ...
                if token == "custom":
                    ...
                    visible_content = stream_sanitizer.feed(metadata[0].content)
                    if not visible_content:
                        continue
                    response_content += visible_content
                    yield {
                        "type": "response_chunk",
                        "timestamp": time.time(),
                        "data": {
