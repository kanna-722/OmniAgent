...
def get_object_name_from_minio_url(url: str) -> str:
    parsed_url = urlparse(url)
    path = parsed_url.path.lstrip('/')
    try:
        bucket_name = app_settings.storage.minio.bucket_name
    except Exception:
        bucket_name = ""
...
def normalize_object_storage_value(value: str) -> str:
    if not value:
        return value
    if not isinstance(value, str):
        return value
    try:
        mode = app_settings.storage.mode
    except Exception:
        return value
    if mode != "minio":
        return value
    if value.startswith("http://") or value.startswith("https://"):
        ...
        if base_netloc and parsed.netloc == base_netloc:
            return get_object_name_from_minio_url(value)
        try:
            if parsed.netloc == app_settings.storage.minio.endpoint:
                return get_object_name_from_minio_url(value)
        except Exception:
            pass
        return value
